...
import { Calendar, ContactRound, Eye } from 'lucide-react'
		...
		<div className='w-full max-w-[400px] flex flex-col gap-3 pb-18'>
			<Card className=''>
				<CardHeader className='p-0'>
					<CardTitle
						className='flex relative gap-2 items-center justify-between h-50 bg-cover bg-center rounded-lg px-5'
						style={{ backgroundImage: `url(${expense.photoUrl})` }}
					>
				...
				<CardContent>
					<ul className='space-y-3'>
						{expense.payers.map(payer => (
							<li key={payer.id} className='flex items-center gap-2'>
								<div className="">
									<Avatar>
										<AvatarImage
											src={
								...
								<div className='w-full'>
									...
									<div className="h-3 relative flex items-center bg-good-green/20 rounded-3xl pl-1">
										<div className="h-1 bg-good-green rounded-l-full" style={{width: `${(payer.amount*100)/expense.amount}%`}}></div>
										<div className="bg-good-green px-1 text-background rounded-full text-xs">{payer.amount}</div>
									</div>
								</div>
							</li>
						))}
						{expense.splits.map(splitter => (
							<li key={splitter.id} className='flex items-center gap-2'>
								<div className="">
									<Avatar>
										<AvatarImage
											src={
								...
								<div className='w-full'>
									...
									<div className="h-3 relative flex items-center bg-bad-red/20 rounded-3xl pl-1" style={{width: `${(splitter.amount*100)/expense.amount}%`}}>
										<div className="h-1 bg-bad-red rounded-l-full w-full"></div>
										<div className="bg-bad-red px-1 text-background rounded-full text-xs">{splitter.amount}</div>
									</div>
								</div>
							</li>
						))}
			...
			<Card>
				<CardContent className='flex justify-center items-center gap-3 pt-3'>
					...
					<Button className='px-5 ' variant={'danger'}>Delete</Button>
				</CardContent>
			</Card>
		</div>
	)
